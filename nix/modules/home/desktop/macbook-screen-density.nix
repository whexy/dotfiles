...
#
# Gated on `dotfiles.hardware.display.macbookScreen` so that standalone Linux machines
...
in
{
  config = lib.mkIf cfg.macbookScreenDensity.enable {
    dconf.settings = lib.mkIf (osConfig.dotfiles.hardware.display.macbookScreen or false) {
