...
# from the upstream repo, via a user-level systemd timer.
#
# Runs the same command documented for manual use, plus --refresh:
#   nh home switch <upstreamRef> --refresh --no-nom -c <user>@<host> -b backup
#
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    systemd.user.services.dotfiles-auto-upgrade = {
      Unit.Description = "Auto-upgrade Home Manager from ${upstreamRef}";
      Service = {
        Type = "oneshot";
    ...
    systemd.user.timers.dotfiles-auto-upgrade = {
      Unit.Description = "Daily Home Manager auto-upgrade from ${upstreamRef}";
      Timer = {
        OnCalendar = "*-*-* 05:00:00";
